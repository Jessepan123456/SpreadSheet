...
{ 
    // This suppresses the "Use Cooperative Cancellation" warning.
    // We explicitly DON'T want to use Cooperative Cancellation: it keeps tests from exiting abruptly.
    // And so it allows tests to run past their timeout, which causes the autograder to fail completely
#pragma warning disable MSTEST0045
    ...
    {
        return new List<string>(sheet.SetContentsOfCell(name, contents));
    }
    ...
    {
        Spreadsheet ss = new Spreadsheet();
        Assert.ThrowsExactly<SpreadsheetReadWriteException>(() => ss.Save( Path.GetFullPath( "/missing/save1.txt" ) ));
    }
    ...
    {
        Assert.ThrowsExactly<SpreadsheetReadWriteException>(() => new Spreadsheet(Path.GetFullPath("/missing/save2.txt")));
    }
